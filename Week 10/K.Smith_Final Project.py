...
while user_choice != "5":
    ...
    if user_choice == "1":
        show_seats(seat_setup) # shows all seats
        ...
        while answer == "y":
            ...
            try:
                ...
                if row not in [1,2,3,4,5,]:
                    row = int(input("INVALID ENTRY!!! Please Choose A Row 1-5: "))
                
            except ValueError:

                row = int(input("Try Again ERROR!! Choose A ROWWW!! 1-5: "))
            ...
            if seat_setup[seat_column][index] != "X":
                ...
                print(f"Congratulations!!! Seat {seat}{row} has been successfully reserved in First Class !!")
            else:
                print(f"This seat is already taken sorry. Please choose another seat.")


            print("\nSeats Available:") #updates map seating
            for index in range(5):  
                # Print row with class designation
                print(f"Row {index + 1:2} {seat_setup[0][index]} {seat_setup[1][index]} {seat_setup[2][index]}   {seat_setup[3][index]} {seat_setup[4][index]} {seat_setup[5][index]}")
