...
#display one dow of data from the file 
display(16,len(name))
...
while ans == "y":
    ...
    if search_type not in ["1", "2", "3", "4"]:
       ...
       #show all values in class_type of either dragon or elf ---> sequential search
        ...
        #Sequenial SEARCH for Multiple Values Matching Searcg term
        else:
            found =[]
            ...
            if not found: #if the found list is still empty
                print(f" Sorry, your search for {search} came up emptu :[")

            else:
                #call displaay to show the values
                display("x",len(found))
        ...
        #A for loop inside a for loop YOU MUST USE ANOTHER LETTER
        for i in range (0,len(name)-1):
            for j in range(0,len(name)-1):
                if name[j] >name[j+1]: #FOR DESCENDING ORDER SWAP THE GREATER TO LESS
                    #they must swap places because the higher vlaue must come afterwards
                    temp = name[j]
                    name[j] =name[j+1]
                    name[j+1] = temp
        ...
        #BINAERY SEARCH must be performed on ordered/sorted lists populated with unique values - can only find ONe item or value.
        ...
        while min < max and search.lower() != name[mid].lower():
            #while above is true, list is not yet exhausted and we haven't found what we are looking for so, 
            #must go through another searching iteration!
        ...
        if search.lower() == name [mid].lower():
            ...
            print(f"Sorry, WE have  notfound your search for {search}, Please Try again.")
    elif search_type == "3":
        ...
        else: print(f"Huzzah!, We have found your seach for {search}, see the details below:")
        display("x", len(found))
